            ...
            async with session.begin():
                ...
                res2 = await session.execute(
                    ...
                    .bindparams(
                        ...
                        stool_count=int((payload.raw_data or {}).get('stool_count', 0)),
                        pads_used=int((payload.raw_data or {}).get('pads_used', 0)),
                        ...
                        food_vegetables_all=int((payload.food_consumption or {}).get('Vegetables (all types)', 0)),
                        food_root_vegetables=int((payload.food_consumption or {}).get('Root vegetables', 0)),
                        food_whole_grains=int((payload.food_consumption or {}).get('Whole grains', 0)),
                        food_whole_grain_bread=int((payload.food_consumption or {}).get('Whole grain bread', 0)),
                        food_nuts_and_seeds=int((payload.food_consumption or {}).get('Nuts and seeds', 0)),
                        food_legumes=int((payload.food_consumption or {}).get('Legumes', 0)),
                        food_fruits_with_skin=int((payload.food_consumption or {}).get('Fruits with skin', 0)),
                        food_berries=int((payload.food_consumption or {}).get('Berries (any)', 0)),
                        food_soft_fruits_no_skin=int((payload.food_consumption or {}).get('Soft fruits without skin', 0)),
                        food_muesli_and_bran=int((payload.food_consumption or {}).get('Muesli and bran cereals', 0)),
                        drink_water=int((payload.drink_consumption or {}).get('Water', 0)),
                        drink_coffee=int((payload.drink_consumption or {}).get('Coffee', 0)),
                        drink_tea=int((payload.drink_consumption or {}).get('Tea', 0)),
                        drink_alcohol=int((payload.drink_consumption or {}).get('Alcohol', 0)),
                        drink_carbonated=int((payload.drink_consumption or {}).get('Carbonated drinks', 0)),
                        drink_juices=int((payload.drink_consumption or {}).get('Juices', 0)),
                        drink_dairy=int((payload.drink_consumption or {}).get('Dairy drinks', 0)),
                        drink_energy=int((payload.drink_consumption or {}).get('Energy drinks', 0)),
                    )
                )
        return {"status": "ok", "id": str(res2.first()[0])}
    except Exception as e:
        return JSONResponse(status_code=500, content={"status": "error", "detail": repr(e)})
